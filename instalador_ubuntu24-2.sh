...
# Ajuste para Ubuntu 24.04 - libsrt-dev não está mais disponível
if grep -q "24.04" /etc/os-release; then
    # Para Ubuntu 24.04, usar libsrt-openssl-dev
...
instala_postgres_base() {
  ...
  # Ajuste para Ubuntu 24.04 - usar método correto para adicionar chave GPG
  if grep -q "24.04" /etc/os-release; then
    # Para Ubuntu 24.04, usar o método mais recente
...
instala_node_base() {
  ...
  {
    sudo su - root <<EOF
  # Método atualizado para instalar Node.js 20.x compatível com Ubuntu 24.04
...
EOF
    ...
    # Ajuste para Ubuntu 24.04 - Certbot via snap funciona melhor
    sudo su - root <<EOF
  apt install -y snapd
  snap install core
  snap refresh core
